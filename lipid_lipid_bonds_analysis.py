...
def plot_bonds(name,kinds,**kwargs):
				...
				for combo in combos:
					...
					if len(rows)>0:
						...
						if combo not in post[sn_group]: 
							...
							post[sn_group][combo] = [[np.zeros(nframes_by_sn[ii]) 
						...
						post[sn_group][combo][knum][snum] = counts
	...
	for sn_group in post:
		for combo in post[sn_group]:
			...
			except: raise Exception('failed to process %s,%s'%(sn_group,combo))
	...
	max_count = int(np.concatenate([np.concatenate(post[sn].values()) for sn in post]).max())
	if style=='imshow':
				...
				for sn in sns_this:
					...
					for bin_num,intensity in zip(bins_which[bins_which>0],bins_intensity[bins_which>0]):
						...
						image[inds[0]:inds[1],inds[2]:inds[3]] = np.array(mpl.colors.to_rgba(
							color_by_simulation(sn)))*intensity
					xpos += width_bins

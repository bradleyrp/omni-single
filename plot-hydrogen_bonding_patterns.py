...
def render_hydrogen_bonding_pattern(fr,frameno):
	...
	if show_proteins:
		plothull(ax,[data_prot[sn]['data']['points'][fr][:,:2]],
			...
			griddims=None,fill=True,lw=0,alpha=0.35,c='k')
	#---plot a box
	lims_prop = 0.1
...
if 'test' in routine:
	...
	if 'data' not in globals():
		...
		dot_size_small = plotspecs.get('dot_size_small',8)
		dot_size_med = plotspecs.get('dot_size_med',16)
		dot_size_large = plotspecs.get('dot_size_large',25)

	#---set smooth_window to zero for no smoothing
	#---! smoothing is broken because PBCs. DO NOT USE SMOOTHING.
	smooth_window = 0
	show_proteins = plotspecs.get('get_protein_abstractor',False)
	if show_proteins: data_prot,calc_prot = work.plotload('protein_abstractor',sns=sns)

	#---loop over simulations
	for sn in sns:
		lipid_mesh = work.plotload('load_mesh',sns=[sn])[0][sn]['data']
		...
		if os.path.isdir(out_dn): status('refusing to overwrite %s'%out_dn,tag='warning')
			...
			#top_mono = work.meta[sn].get('index_top_monolayer',0)
			#---override the top_mono by checking it manually
			avg_z = [data['lipid_abstractor'][sn]['data']['points'].mean(axis=0)[j].mean(axis=0)[2] 
				for j in [np.where(data['lipid_abstractor'][sn]['data']['monolayer_indices']==i) 
				for i in [0,1]]]
			top_mono = work.meta[sn].get('index_top_monolayer',np.argmax(avg_z))
			...
			basic_compute_loop(
				...
				looper=[dict(fr=fr,frameno=frameno) for frameno,fr in enumerate(valid_frames)],
				run_parallel=True,debug=False)
